...
class AgilexDataConfig:
    ...
    action_keys = [
        "action.left_joints",
        "action.right_joints",  # This order differs from the source dataset.
        "action.left_gripper",
        "action.right_gripper",
    ]
...
class RealWorldM2WStride2DataConfig(AgilexDataConfig):
    # 16 consecutive action targets: t, t+1, ..., t+15.
    # Wrist/V-JEPA clips still use temporal stride in the M2W dataloader.
    action_indices = list(range(16))

    def transform(self):
        # Real-world Piper grippers are continuous low-range values (about 0.0-0.1),
        # so binary thresholding would collapse them to zero. Keep them min-max scaled.
        transforms = [
            StateActionToTensor(apply_to=self.state_keys),
            StateActionTransform(
                apply_to=self.state_keys,
...
ROBOT_TYPE_CONFIG_MAP = {
    ...
    "realworld_m2w_stride2": RealWorldM2WStride2DataConfig(),
