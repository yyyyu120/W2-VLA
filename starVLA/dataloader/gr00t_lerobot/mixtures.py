...
# Clean-only Robotwin with a 50-step action chunk. It reuses the same bare task
# directories as "robotwin", but switches the robot config to action_indices=50.
...
# Clean-only Robotwin with a 16-step action chunk. It reuses the same bare task
# directories as "robotwin_clean_50", but keeps robotwin action_indices=16.
DATASET_NAMED_MIXTURES["robotwin_clean_h16"] = [
    (dataset_name, weight, "robotwin")
    for dataset_name, weight, _robot_type in DATASET_NAMED_MIXTURES["robotwin"]
]

# Real-world M2W place-bag demo. The dataset is LeRobot/RoboTwin-style with
# high/left-wrist/right-wrist videos and 14-D bimanual actions. The robot config
# keeps a 16-step action chunk but samples every 2 frames.
DATASET_NAMED_MIXTURES["realworld_place_bag_stride2"] = [
    ("place_bag", 1.0, "realworld_m2w_stride2"),
]
# Real-world M2W put-mango demo. Same LeRobot/RoboTwin-style layout as
# place-bag: high/left-wrist/right-wrist videos and 14-D bimanual actions.
DATASET_NAMED_MIXTURES["realworld_put_mango_stride2"] = [
    ("put_mango", 1.0, "realworld_m2w_stride2"),
]

# Real-world M2W table-cleaning demo. It uses the same tri-view bimanual
# layout, 16-step action chunk, and stride-2 wrist/V-JEPA clips.
DATASET_NAMED_MIXTURES["realworld_table_clean_stride2"] = [
    ("table_clean", 1.0, "realworld_m2w_stride2"),
]

# Real-world M2W plug-in-socket demo. It uses the same tri-view bimanual
# layout, 16-step action chunk, and stride-2 wrist/V-JEPA clips.
DATASET_NAMED_MIXTURES["realworld_plug_in_socket_stride2"] = [
    ("plug_in_socket", 1.0, "realworld_m2w_stride2"),
]
