...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        self.model.gradient_checkpointing = bool(config.get("gradient_checkpointing", False))
        self.action_dim = config.action_dim
        action_horizon = config.get("action_horizon", None)
        if action_horizon is None:
            action_horizon = int(config.future_action_window_size) + 1
        self.action_horizon = int(action_horizon)
        config.action_horizon = self.action_horizon
        config.future_action_window_size = self.action_horizon - 1
