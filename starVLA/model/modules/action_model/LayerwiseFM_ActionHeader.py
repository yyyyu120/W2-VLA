...
class LayerwiseFlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Propagate DiTConfig into diffusion_model_cfg.
    ...
    def forward(
        ...
        """
        vl_embs_list: list of torch.Tensor, each shape (B, seq_length, feature_dim)
        actions:      shape (B, future_action_window_size, D_action)
        state:        optional proprioceptive state
        prompt_tokens: optional dict with keys "g" and/or "e", each [B, P, D].
                       Injected as extra cross-attention context at specified layers.
                       Controlled by g_inject_layers / e_inject_layers in config.
        """
        ...
        # Parse prompt injection config
        g_inject = set(getattr(self.config, "g_inject_layers", []))
        e_inject = set(getattr(self.config, "e_inject_layers", []))
        g_tokens = prompt_tokens.get("g") if prompt_tokens else None  # [B, Pg, D]
        e_tokens = prompt_tokens.get("e") if prompt_tokens else None  # [B, Pe, D]

        # Layerwise cross-attention with vl_embs + optional prompt injection
        model_output = sa_embs
        for layer_idx, layer in enumerate(self.model.transformer_blocks):
            ctx = vl_embs_list[layer_idx]  # [B, L, D]
            # Append prompt tokens to cross-attention context at specified layers
            extra = []
            if g_tokens is not None and layer_idx in g_inject:
                extra.append(g_tokens)
            if e_tokens is not None and layer_idx in e_inject:
                extra.append(e_tokens)
            if extra:
                ctx = torch.cat([ctx] + extra, dim=1)  # [B, L+Pg+Pe, D]
            model_output = layer(
    ...
    def predict_action(
        ...
        g_inject = set(getattr(self.config, "g_inject_layers", []))
        e_inject = set(getattr(self.config, "e_inject_layers", []))
        g_tokens = prompt_tokens.get("g") if prompt_tokens else None
        e_tokens = prompt_tokens.get("e") if prompt_tokens else None
        ...
        for t in range(num_steps):
            ...
            # Layerwise cross-attention with vl_embs_list + optional prompt injection
            model_output = sa_embs
            for layer_idx, layer in enumerate(self.model.transformer_blocks):
                ctx = vl_embs_list[layer_idx]
                extra = []
                if g_tokens is not None and layer_idx in g_inject:
                    extra.append(g_tokens)
                if e_tokens is not None and layer_idx in e_inject:
