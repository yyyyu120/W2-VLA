...
# TODO: Merge the YAML configuration with the Transformers configuration.
...
class baseframework(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if getattr(model_config.framework, "name", "") == "QwenSubtaskM2W":
            removed_residual_keys = [
                key for key in model_state_dict if key.startswith("m2w_adapter.action_residual_")
            ]
            if removed_residual_keys:
                logger.warning(
                    "Dropping removed M2W residual keys from checkpoint: "
                    f"{sorted(removed_residual_keys)}"
                )
                for key in removed_residual_keys:
