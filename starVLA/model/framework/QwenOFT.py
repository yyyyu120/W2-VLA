...
class Qwenvl_OFT(baseframework):
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.float32):
            # Use action-token embeddings as action prediction queries.
    ...
    def _gather_action_token_embeddings(
        ...
        Vectorized extraction of action-token embeddings:
          - avoids a per-sample Python loop;
          - selects the final chunk_len action placeholder tokens.
        Args:
            last_hidden: [B, L, H]
            input_ids:   [B, L]
        ...
        if action_token_id is None:
            raise ValueError("action_token_id must not be empty")

        device = input_ids.device
        B, L, H = last_hidden.shape

        # Support multiple token IDs.
        ...
        if (counts < self.chunk_len).any():
            ...
            raise RuntimeError(
                f"Samples have fewer than {self.chunk_len} action tokens: "
        ...
        # Select the final chunk_len positions. Counts were validated above,
        # so masked -1 positions cannot be selected.
