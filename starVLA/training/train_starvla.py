...
gradient_accumulation_plugin = GradientAccumulationPlugin(
    ...
    # Plain DDP can keep the usual no_sync behavior for faster accumulation.
...
class VLATrainer(TrainerUtils):
    ...
    def prepare_training(self):
        ...
        # Advance the learning-rate scheduler when resuming.
    ...
    def _adjust_lr_scheduler_for_resume(self):
        """Advance the learning-rate scheduler to the completed step."""
        if self.completed_steps > 0:
            logger.info(f"Adjusting LR scheduler for resume from step {self.completed_steps}")
            
            # Simulate the completed steps; this works for most schedulers.
            for _ in range(self.completed_steps):
                self.lr_scheduler.step()
            
            # Some schedulers can instead restore their final step directly.
    ...
    def _init_checkpointing(self):
        ...
        # Resolve the pretrained checkpoint and resume flag.
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            # With gradient accumulation, optimizer.step() is skipped on
            # non-sync micro-steps, but lr_scheduler.step() would otherwise
            # still advance and make the LR schedule decay too fast.
            if self.accelerator.sync_gradients:
                self.lr_scheduler.step()
