...
class ModelClient:
    def __init__(
        ...
        self.wrist_history_frames = self.get_wrist_history_frames(policy_ckpt_path)
        self.wrist_image_history = deque(maxlen=max(1, self.wrist_history_frames))
    ...
    def step(self, example: dict, step: int = 0, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        if self.wrist_history_frames > 1 and len(images) >= 2:
            self.wrist_image_history.append(images[1])
            wrist_history = list(self.wrist_image_history)
            if wrist_history:
                pad = [wrist_history[0]] * max(0, self.wrist_history_frames - len(wrist_history))
                example["wrist_views"] = [pad + wrist_history]
        vla_input = {
            "examples": [example],
            "do_sample": False,
