...
class Args:
    ...
    task_id_start: int = 0  # Inclusive task id start for sharded evaluation.
    task_id_end: int = -1  # Exclusive task id end for sharded evaluation. -1 means all remaining tasks.
...
def eval_libero(args: Args) -> None:
    ...
    if task_id_start >= task_id_end:
        raise ValueError(
            f"Invalid task range [{args.task_id_start}, {args.task_id_end}) "
            f"for suite {args.task_suite_name} with {num_tasks_in_suite} tasks"
        )
    logging.info(f"Task id range: [{task_id_start}, {task_id_end}) / {num_tasks_in_suite}")
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            logging.info(
                client_model.format_latency_summary(
                    ...
                    client_model.get_episode_latency_summary(),
                )
            )
