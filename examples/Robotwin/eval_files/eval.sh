...
if [[ -z "${ROBOTWIN_PATH}" ]]; then
    echo "Set ROBOTWIN_PATH to your RoboTwin-Platform checkout." >&2
    exit 1
fi
...
if [[ "${policy_ckpt_path}" != /* ]]; then
    policy_ckpt_path="${REPO_ROOT}/${policy_ckpt_path}"
fi
...
extra_overrides=()
if [[ -n "${ROBOTWIN_TEST_NUM:-}" ]]; then
    extra_overrides+=(--test_num "${ROBOTWIN_TEST_NUM}")
fi
if [[ -n "${ROBOTWIN_EVAL_VIDEO_LOG:-}" ]]; then
    extra_overrides+=(--eval_video_log "${ROBOTWIN_EVAL_VIDEO_LOG}")
fi
if [[ -n "${ROBOTWIN_START_SEED:-}" ]]; then
    extra_overrides+=(--start_seed "${ROBOTWIN_START_SEED}")
fi
