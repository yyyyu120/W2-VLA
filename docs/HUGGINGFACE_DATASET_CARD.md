...
Each `episode_XXXXXX.npz` file contains episode metadata and frame-aligned CoT
annotations. The default W2-VLA training pipeline uses only the
`cot_train_text` field for language supervision, formatted as:
...
`num_frames`. The decomposed fields are provided for inspection, filtering,
and ablation; they are not separately consumed by the default trainer.
